...
Standard commands are defined in `package.json` (`dev`, `build`, `start`, `lint`, `token:generate`, `smoke:write-api`, `test:document-ref`):
...
- No full test suite (no Jest/Vitest/Playwright). Contract regression for the write API: `npm run smoke:write-api`. DocumentRef unit tests: `npm run test:document-ref`. Also use `npm run lint`, `npx tsc --noEmit`, `npm run build`, and manual terminal UI checks.
