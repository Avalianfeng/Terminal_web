...
**ContentVisibility / publish（ADR 0018 + 0019 + 0021）**:
Archive body (`.md` / `person.json` / `timeline.md`) is **not** in the public Git remote; playlist curation yaml stays tracked. Zones are structural: `content/<group>/` = public, `content/private/<group>/` = private (not frontmatter). **Authority for body is the production `content/` tree** (including `private/`); local is upload-only, not a mirror (ADR 0021). Runtime: capability lattice + `getArchiveSnapshotFor(grant)` (ADR 0019). `selectPublishPaths` **still excludes** `private/**` until the ops console is upload-only. Member **semantics** locked (read private); member login Planned. Password-session Grant tightening: ADR 0022 (not implemented). Bearer scope is write-only; valid token reads all.
_Avoid_: treating GitHub as the body CDN; frontmatter `visibility` as authority; sparse-tree `rsync --delete` as a full mirror; 403 for unreachable docs; treating laptop `content/` as the backup
